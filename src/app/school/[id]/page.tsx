      ...
      <div className="flex flex-col min-h-screen">
        <main className="flex-grow flex items-center justify-center px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="mb-6">The school you are looking for does not exist or has been removed.</p>
            <Button asChild>
        ...
        <main className="flex-grow">
          <div className="bg-hero-custom text-white py-24 px-4 sm:px-6 lg:px-8">
            <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-6">
              <Button variant="ghost" onClick={() => router.back()} className="absolute top-4 left-4 text-white">
                &larr; Back
              </Button>
              ...
              <div className="text-center md:text-left text-muted-foreground">
                ...
                {school.description && <p className="mb-4 text-sm sm:text-base md:text-xl max-w-2xl mx-auto md:mx-0">{school.description}</p>}
                {school.website && (
            ...
            <Tabs defaultValue="overview">
              ...
              <TabsContent value="overview" className="space-y-8">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 text-black ">
                  <StatCard icon={<Building className="h-5 w-5  " />} title="Type" value={school.type} />
                  {school.ranking != null && (
                    <StatCard icon={<Award className="h-5 w-5 " />} title="Ranking" value={`#${school.ranking}`} />
                  )}
                  {school.enrollmentSize != null && (
                    <StatCard icon={<Users className="h-5 w-5 " />} title="Enrollment" value={school.enrollmentSize.toLocaleString()} />
                  )}
                  {school.acceptanceRate != null && (
                    <StatCard icon={<GraduationCap className="h-5 w-5 " />} title="Acceptance Rate" value={`${(school.acceptanceRate * 100).toFixed(1)}%`} />
                  )}
                </div>
                ...
                <div>
                  ...
                  <p className="mb-4">
                    {school.name} offers {scholarships.length}{' '}
                    scholarship{scholarships.length !== 1 ? 's' : ''} for international students.
                  </p>
              ...
              <TabsContent value="tuition">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  <TuitionCard title="In-State Tuition" amount={school.tuition.inState} description="For residents of the state" />
                  <TuitionCard title="Out-of-State Tuition" amount={school.tuition.outOfState} description="For U.S. residents from other states" />
                  <TuitionCard title="International Tuition" amount={school.tuition.international} description="For international students" highlighted />
                </div>
                <div className="mt-8 p-4 bg-scholar-50 rounded-lg border border-scholar-100">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h3 className="font-medium text-scholar-800">Additional Costs to Consider</h3>
                      <p className="text-sm text-muted-foreground mt-1">
                        Total cost of attendance includes tuition, fees, room & board, books, supplies, transportation, personal expenses, and for international students: travel, visa fees, and health insurance.
                      </p>
                    </div>
                  </div>
                </div>
...
const TuitionCard = ({ title, amount, description, highlighted = false }: { title: string; amount: number; description: string; highlighted?: boolean; }) => (
  <div className={`p-6 rounded-lg border ${highlighted ? 'bg-white border-scholar-200 shadow-md' : 'bg-gray-50 border-gray-100'}`}>
    <h3 className={`font-medium mb-1 ${highlighted ? 'text-scholar-700' : ''}`}>{title}</h3>
